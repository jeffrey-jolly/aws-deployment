...
export class AuthService{
  ...
  login(email: string, password){
    ...
    this.http.post<{token: string, expiresIn: number, role :string , message: string}>("api/user/login",authData)
      .subscribe(response =>{
  ...
  getUser() {
    this.http.get<{message: string, users: any}>('api/user/getUserData')
    .pipe(map(userData => {
